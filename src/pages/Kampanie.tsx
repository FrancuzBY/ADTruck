    ...
    <div className="min-h-full bg-gradient-to-b from-white to-canvas px-5 pt-[76px] pb-4">
      ...
      {campaigns.length === 0 ? (
        <div className="mt-16 flex flex-col items-center text-center">
          <div className="grid size-[66px] place-items-center rounded-[20px] bg-brand-soft text-brand">
          ...
          <Link
            to="/zamow"
            className="mt-6 rounded-full bg-cta px-7 py-3.5 text-[15.5px] font-semibold text-white shadow-[0_12px_28px_-10px_rgb(22_163_74_/_0.55)] transition-colors hover:bg-cta-strong"
          >
        ...
        <div className="mt-5 space-y-3">
          {campaigns.map((c, i) => (
            ...
            >
              <Link
                to={`/kampanie/${c.id}`}
                className="block rounded-[20px] border border-line bg-surface p-[18px] shadow-card transition-transform hover:-translate-y-0.5"
              >
          ...
          <Link
            to="/zamow"
            className="mt-4 block w-full rounded-full bg-brand-soft py-4 text-center text-[15px] font-semibold text-brand transition-[filter] hover:brightness-[0.97]"
          >
            Nowa kampania
          </Link>
        </div>
